...
import { hashPassword, verifyPassword } from '../../server/src/validations/password_hash';
...
describe('Test for Password Hash', () => {
  ...
  describe('## Verify the Password Hash', () => {
    ...
    it('should verify the password hash and return a boolean(true)', async () => {
      const result = await verifyPassword(password, userPassword);
    ...
    it('should verify the password hash and return a boolean(false)', async () => {
      ...
      const result = await verifyPassword(wrongPassword, userPassword);
      expect(result).to.be.a('boolean');
      expect(result).to.equal(false);
    });

    it('should not verify the password and return an Error(Database Password must be Provided for Comparism)', () => verifyPassword(password).should.be.rejectedWith(Error, 'Database Password must be Provided for Comparism'));

    it('should not verify the password and return a TypeError(Password Must Be a String)', () => verifyPassword(badPassword, userPassword).should.be.rejectedWith(TypeError, 'Password Must Be a String'));
  });
});
